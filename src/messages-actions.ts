...
export async function forwardMessage(id: string, content: string, senderName: string) {
    ...
    renderedChats.forEach((chat: any) => {
        ...
        const premiumBadgeHtml = isPremiumUser ? `<div class="absolute -top-1 -left-1 bg-white dark:bg-gray-800 rounded-full p-0.5 shadow-sm border border-gray-200 dark:border-gray-700 z-50 w-4 h-4 flex items-center justify-center"><img src="./image/Google-Gemini-Logo-Transparent.png" referrerpolicy="no-referrer" class="w-full h-full object-contain" alt="Premium"></div>` : '';
        ...
        if (isSavedMessages) {
            ...
            avatarHtml = avatarUrl 
                ? `<div class="w-full h-full rounded-full overflow-hidden relative"><img src="${avatarUrl}" referrerpolicy="no-referrer" class="w-full h-full object-cover"></div>` 
