...
export async function loadChats() {
  try {
    ...
    // Find Saved Messages
    let savedChats = chats.filter(
      (c: any) => c.description === "SAVED_MESSAGES" && c.type === "private",
    );
    let fallbackChats = chats.filter(
      (c: any) =>
        c.type === "private" &&
        (!c.chat_members ||
    ...
    if (savedChats.length > 0) {
      savedMessagesChat = savedChats[0];
    } else if (fallbackChats.length > 0) {
      savedMessagesChat = fallbackChats[0];
    }

    if (savedMessagesChat) {
      const idx = chats.findIndex((c: any) => c.id === savedMessagesChat.id);
      if (idx !== -1) chats.splice(idx, 1);

      // Lock this chat explicitly as Saved Messages
      if (
        savedMessagesChat.description !== "SAVED_MESSAGES" ||
        savedMessagesChat.title !== "Избранное"
      ) {
        savedMessagesChat.description = "SAVED_MESSAGES";
        savedMessagesChat.title = "Избранное";
        supabase
    ...
    // Clean up any remaining duplicate Saved Messages (ghosts)
    chats.forEach((c: any) => {
      if (c.description === "SAVED_MESSAGES" && c !== savedMessagesChat) {
        c.description = null;
        if (c.title === "Избранное") c.title = null;
        supabase
          .from("chats")
    ...
    chats.forEach((chat: any) => {
      ...
      let isSavedMsgs =
        chat === savedMessagesChat || chat.id === "new_saved_messages";
