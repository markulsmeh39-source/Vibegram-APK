...
export function searchUsers(q: string) {
  ...
  searchTimeout = setTimeout(async () => {
      ...
      if (channel) {
        ...
        div.onclick = () => {
          ...
          (document.getElementById("search-input") as HTMLInputElement).value =
            "";
          joinChannelWithKey(channel, q);
        };
        resultsBox.appendChild(div);
        resultsBox.classList.remove("hidden");
      } else {
        resultsBox.innerHTML =
          '<div class="p-4 text-sm text-gray-500 text-center font-medium">Ключ не найден или уже использован</div>';
        resultsBox.classList.remove("hidden");
      }
      return;
    }
    ...
    const { data: groups } = await supabase
      ...
      .eq("type", "group")
      .eq("is_public", true)
      .or(`title.ilike.%${searchTerm}%,username.ilike.%${searchTerm}%`)
      .limit(10);
    ...
    const users = usersRaw
      ?.filter((u) => !(u.settings && u.settings.is_tech_support))
      ?.slice(0, 10);
    ...
    if (
      ...
      resultsBox.innerHTML =
        '<div class="p-4 text-sm text-gray-500 text-center font-medium">Ничего не найдено</div>';
    } else {
      resultsBox.innerHTML = "";
      users?.forEach((u) => {
...
export async function joinGroup(group: any) {
  ...
  if (existing) {
    if (existing.role === "pending") {
      import("./utils").then((m) =>
        m.customAlert(
          "Заявка на вступление уже отправлена. Ожидайте подтверждения.",
        ),
      );
    } else {
      const { data: members } = await supabase
    ...
    import("./utils").then((m) =>
      m.customAlert("Заявка на вступление отправлена."),
    );
  }
}
...
export async function startChatWithUser(userToFind: any) {
  ...
  if (isSelf) {
    ...
    if (myChatIds.length > 0) {
      const { data: allMembers } = await supabase
        ...
        .in("chats.type", ["private", "direct"]);
      const counts: Record<string, number> = {};
      allMembers?.forEach(
        (m) => (counts[m.chat_id] = (counts[m.chat_id] || 0) + 1),
      );
  ...
  if (!chatId) {
    ...
    const { error: chatErr } = await supabase
      .from("chats")
      .insert({ id: newChatId, type: "private" });
    ...
    const membersToInsert = isSelf
      ...
      : [
          { chat_id: chatId, user_id: state.currentUser.id },
          { chat_id: chatId, user_id: userToFind.id },
        ];
  ...
  openChat(
    chatId,
    isSelf ? "Избранное" : userToFind.display_name || userToFind.username,
    (userToFind.display_name || userToFind.username)[0].toUpperCase(),
    false,
    "private",
    [{ user_id: userToFind.id, profiles: userToFind }],
    userToFind.avatar_url,
  );
...
export async function openUserProfile(userId: string) {
  ...
  let isPremiumUser =
    ...
    (!userToFind.premium_until ||
      new Date(userToFind.premium_until) > new Date());
  const premiumBadgeHtml = isPremiumUser
  ...
  const avatarInnerHtml = userToFind.avatar_url
    ? `<img src="${userToFind.avatar_url}" class="w-full h-full object-cover rounded-full">`
    : `${(userToFind.display_name || userToFind.username || "U")[0].toUpperCase()}`;
                ...
                ${
                  userToFind.id !== state.currentUser?.id
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        ${
                          userToFind.username
                            ? `<button onclick="if(window.sendVibToUserModal) { window.sendVibToUserModal('${userToFind.username}'); }" class="w-full bg-gradient-to-r from-indigo-500 to-purple-600 shadow-md hover:shadow-lg text-white font-medium py-3 px-4 rounded-xl active:scale-[0.98] transition-all flex items-center justify-center gap-2">
