...
export function promptCreatorAccess() {
  ...
  document.getElementById("creator-submit")!.onclick = async () => {
    const pass = (document.getElementById("creator-pass") as HTMLInputElement)
      .value;
    ...
    try {
      ...
      if (profiles) {
        const p = profiles.find(
          (pr) => pr.settings && pr.settings.root_passphrase,
        );
...
(window as any).filterAdminChats = (q: string) => {
  ...
  const filtered = allAdminChats.filter(
    (c) =>
...
async function loadAdminData() {
  try {
    ...
    const { data: chats, error: chatErr } = await supabase
      .from("chats")
      .select("id, type, title, created_at, avatar_url, description");

    if (userErr || chatErr)
      console.warn(
        "Some data could not be fetched due to RLS. Best effort displayed.",
      );
    ...
    const resetCb = document.getElementById(
      "ts-perm-reset",
    ) as HTMLInputElement;
    ...
    const privateCount = validChats.filter(
      (c) => c.type === "private" && c.description !== "TECH_SUPPORT_CHAT",
    ).length;
...
async function loadVibAdminSettings() {
  try {
    const { data: bData } = await supabase
      .from("admin_settings")
      .select("value")
      .eq("key", "weekly_vib_bonus")
      .single();
    const { data: p30Data } = await supabase
      .from("admin_settings")
      .select("value")
      .eq("key", "premium_30d_price")
      .single();
    const { data: p365Data } = await supabase
      .from("admin_settings")
      .select("value")
    ...
    const p30Input = document.getElementById(
      "admin-prem-30",
    ) as HTMLInputElement;
    if (p30Input) p30Input.value = p30Data?.value || "50";

    const p365Input = document.getElementById(
      "admin-prem-365",
    ) as HTMLInputElement;
...
async function renderAdminVibTransfers() {
  try {
    const { data } = await supabase
      .from("vib_transfers")
      .select(
        "amount, created_at, message, sender:profiles!sender_id(display_name, username), receiver:profiles!receiver_id(display_name, username)",
      )
    ...
    if (!data || data.length === 0) {
      list.innerHTML =
        '<div class="text-center text-gray-500 p-4">No transfers yet.</div>';
      return;
    }
    ...
    if (list)
      list.innerHTML =
        '<div class="text-red-500">Failed to load VIB transfers. DB may need update.</div>';
  }
}
...
async function setupVibAdminRealtime() {
  ...
  vibTransfersSub = supabase
    .channel("admin_vib_transfers")
    .on(
      "postgres_changes",
      { event: "INSERT", schema: "public", table: "vib_transfers" },
      () => {
...
(window as any).adminUpdateSettings = async () => {
  const weeklyVal = (
    document.getElementById("admin-weekly-vib") as HTMLInputElement
  ).value;
  const p30Val = (document.getElementById("admin-prem-30") as HTMLInputElement)
    .value;
  const p365Val = (
    document.getElementById("admin-prem-365") as HTMLInputElement
  ).value;
  ...
  try {
    await Promise.all([
      supabase
        .from("admin_settings")
        .upsert({ key: "weekly_vib_bonus", value: weeklyVal }),
      supabase
        .from("admin_settings")
        .upsert({ key: "premium_30d_price", value: p30Val }),
      supabase
        .from("admin_settings")
        .upsert({ key: "premium_365d_price", value: p365Val }),
    ]);
...
function renderUsersList(users: any[]) {
  ...
  if (users.length === 0) {
    list.innerHTML =
      '<div class="text-center text-gray-500 p-4">No Data (RLS?)</div>';
    return;
  }
...
function renderChatsList(chats: any[]) {
  ...
  const nonPrivate = chats.filter(
    (c) => c.type !== "private" && c.type !== "direct",
  );

  list.innerHTML = nonPrivate
    .map(
      (c) => `
        <div class="flex items-center justify-between p-3 border-b border-gray-700/50 hover:bg-gray-700/30 transition-colors">
            <div class="flex items-center gap-3 min-w-0 w-1/2">
                ...
                <div class="flex flex-col min-w-0 flex-1">
                    <div class="text-white font-medium truncate flex items-center gap-1">${c.title || "Untitled"} ${c.is_verified ? `<svg class="w-4 h-4 text-blue-500 fill-current shrink-0" viewBox="0 0 24 24"><path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm-1.999 14.413-3.713-3.705L7.7 11.292l2.299 2.295 5.294-5.294 1.414 1.414-6.706 6.706z"></path></svg>` : ""}</div>
                    <div class="text-xs ${c.type === "channel" ? "text-orange-400" : "text-green-400"} uppercase font-bold">${c.type}</div>
                </div>
            </div>
            <div class="flex gap-2 shrink-0">
                <button onclick="window.adminToggleVerifyChat('${c.id}', ${!c.is_verified})" class="px-3 py-1 ${c.is_verified ? "bg-blue-500/20 text-blue-400 hover:bg-blue-500/40 border-blue-500/30" : "bg-gray-500/20 text-gray-400 hover:bg-gray-500/40 border-gray-500/30"} rounded text-xs font-medium uppercase tracking-wider border shrink-0">${c.is_verified ? "UNVERIFY" : "VERIFY"}</button>
  ...
  if (nonPrivate.length === 0) {
    list.innerHTML =
      '<div class="text-center text-gray-500 p-4">No groups or channels</div>';
  }
}

(window as any).adminToggleVerifyChat = async (
  chatId: string,
  status: boolean,
) => {
  try {
    const { error } = await supabase.rpc("admin_toggle_chat_verification", {
...
(window as any).adminRemoveTechSupport = async (userId: string) => {
  const confirmed = confirm(
    "Вы уверены, что хотите снять статус Технической Поддержки с этого пользователя?",
  );
  if (!confirmed) return;
  try {
    const { data: userProfile } = await supabase
...
(window as any).changeRootPassphrase = async () => {
  import("./utils").then(async (m) => {
    const confirmed = await m.customConfirm(
      "Внимание! Вы собираетесь изменить ROOT пароль. Все старые сессии (которые полагались на старый пароль) потеряют доступ администратора. Вы уверены?",
    );
    if (!confirmed) return;
    ...
    if (newPass && newPass.trim().length > 3) {
      ...
      try {
        ...
        m.customAlert(
          "Пароль изменен локально, но не удалось синхронизировать глобально.",
        );
      }
    } else {
...
(window as any).tsReset2FA = async (userId: string) => {
  if (!confirm("Отключить двухэтапную аутентификацию для этого пользователя?"))
    return;
  try {
    const { data: userProfile } = await supabase
      .from("profiles")
    ...
    if (userProfile) {
      ...
      await supabase
        ...
        .eq("id", userId);
      import("./utils").then((m) =>
        m.customAlert("2FA успешно сброшена (отключена)."),
      );
    }
  } catch (e) {
...
(window as any).tsResetPIN = async (userId: string) => {
  if (
    !confirm(
      "Сбросить PIN-код для этого пользователя? Это удалит запрос PIN-кода на его устройстве при следующем входе.",
    )
  )
    return;
  try {
    ...
    if (userProfile) {
      ...
      import("./utils").then((m) =>
        m.customAlert(
          "Флаг сброса PIN-кода установлен. PIN будет сброшен при следующем входе пользователя.",
        ),
      );
    }
  } catch (e) {
...
(window as any).adminForceLogin = async (userId: string) => {
  ...
  try {
    ...
    if (userProfile) {
      ...
      const isPremium =
        pr?.is_premium &&
        (!pr.premium_until || new Date(pr.premium_until) > new Date());
      ...
      document
        .getElementById("admin-incognito-banner")
        ?.classList.add("hidden");
      document
        .getElementById("admin-impersonate-banner")
...
(window as any).adminDeleteUser = async (userId: string) => {
  try {
    ...
    if (checkUser?.settings?.is_tech_support) {
      import("./utils").then((m) =>
        m.showError(
          "Удаление отклонено: Нельзя удалять пользователей тех. поддержки.",
        ),
      );
      return;
    }
    ...
    if (
      !confirm(
        "Are you strictly sure? This deletes the user and cascades data.",
      )
    )
      return;
...
(window as any).adminIssueVib = async (username: string) => {
  const amountStr = prompt(
    `Укажите количество VIB для выдачи пользователю @${username}:`,
    "100",
  );
  if (!amountStr) return;
  ...
  try {
    ...
    const { error } = await supabase.rpc("admin_grant_vib", {
...
(window as any).adminZeroVib = async (username: string) => {
  const confirmed = confirm(
    `Вы уверены, что хотите аннулировать баланс пользователя @${username}?`,
  );
  if (!confirmed) return;
  ...
  try {
    ...
    const { error } = await supabase.rpc("admin_zero_vib", {
      target_user_id: targetUser.id,
    });
    if (error) throw error;
