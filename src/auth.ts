...
export async function loginWithGoogle() {
  ...
  if (btn) {
    ...
    document
      .getElementById("auth-loading-indicator")
      ?.classList.remove("hidden");
  }
  try {
    const clientId =
      "362424832513-mdflqja6lr0jq81es5frq66vqic6i1n9.apps.googleusercontent.com";

    let exactRedirectUrl = window.location.origin + window.location.pathname;
    const { Capacitor } = await import("@capacitor/core");

    if (Capacitor.isNativePlatform()) {
      exactRedirectUrl = "https://markulsmeh39-source.github.io/Vibegram-APK/";
    }

    const rawNonce = btoa(
      String.fromCharCode(...crypto.getRandomValues(new Uint8Array(31))),
    ).replace(/=/g, "");
    ...
    import("./utils").then((m) =>
      m.showError("Ошибка входа через Google: " + err.message),
    );
  }
}
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message === "Failed to fetch") {
        import("./utils").then((m) =>
          m.showError(
            "Не удалось подключиться к серверу (Failed to fetch). Возможно, база данных Supabase остановлена.",
          ),
        );
      }
      return;
    ...
    if (user) {
      ...
      if (!isDeviceRegistered) {
      ...
      } else if (accountAgeMs < 120000 && registeredUserId !== user.id) {
        // 2 minutes
        import("./utils").then((m) =>
          m.showError(
            "На этом устройстве уже зарегистрирован аккаунт. Создание новых аккаунтов на одном устройстве запрещено. Вы можете войти только в уже существующий аккаунт.",
          ),
        );
        ...
        if (errElement)
          errElement.innerText =
            "Создание новых аккаунтов на этом устройстве запрещено. Вы можете войти только в уже существующий аккаунт.";
      ...
      if (savedPushToken && data.push_token !== savedPushToken) {
        supabase
          .from("profiles")
          .update({ push_token: savedPushToken })
          .eq("id", user.id)
          .then();
      }
      ...
      if (
        twoStepPasscode &&
        !localStorage.getItem("vibegram_2fa_trusted_" + user.id)
      ) {
        ...
        appLock &&
        !sessionStorage.getItem("vibegram_applock_passed")
      ) {
    ...
    if (
      ...
      import("./utils").then((m) =>
        m.showError(
          "Не удалось подключиться к серверу (Failed to fetch). Возможно, проект Supabase приостановлен.",
        ),
      );
    } else {
      import("./utils").then((m) =>
        m.showError("Ошибка сети. Проверьте подключение или статус Supabase."),
      );
    }
  }
}
  ...
  if (input === state.pendingLockValue) {
    ...
    if (state.pendingLockType === "2fa") {
      localStorage.setItem(
        "vibegram_2fa_trusted_" + state.currentUser?.id,
        "true",
      );
...
export async function resendRecoveryCode() {
  ...
  try {
    ...
    document.getElementById("lock-error")!.className =
      "text-green-500 text-sm mt-3 h-4";
    setTimeout(() => {
      document.getElementById("lock-error")!.className =
        "text-red-500 text-sm mt-3 h-4";
      document.getElementById("lock-error")!.innerText = "";
    }, 3000);
  } catch (err: any) {
    document.getElementById("lock-error")!.innerText =
...
export async function startRecovery(skipSend = false) {
  ...
  showLockScreen(
    "Восстановление",
    `Введите код, отправленный на ${state.currentUser.email}`,
  );
  (document.getElementById("lock-input") as HTMLInputElement).value = "";
  (document.getElementById("lock-input") as HTMLInputElement).placeholder =
    "8-значный код";
  document.getElementById("lock-error")!.innerText = "";
  document.getElementById("lock-forgot-btn")?.classList.add("hidden");
  ...
  if (!skipSend) {
    try {
      ...
      document.getElementById("lock-error")!.innerText =
        "Ошибка отправки кода: " + (err.message || String(err));
    }
  }
...
export async function verifyRecoveryCode() {
  if (!state.currentUser?.email) return;
  const code = (document.getElementById("lock-input") as HTMLInputElement)
    .value;
  ...
  try {
    ...
    document.getElementById("lock-error")!.innerText =
      "Неверный код: " + (err.message || String(err));
  }
}
...
function finalizeAppSetup() {
  ...
  document.getElementById("my-nickname")!.innerHTML =
    `<span class="flex items-center">${nickname}${badge}</span>`;

  const avatarUrl = state.currentProfile?.avatar_url;
  document.getElementById("my-avatar")!.innerHTML =
    `${avatarUrl ? `<img src="${avatarUrl}" class="w-full h-full object-cover rounded-full">` : nickname.charAt(0).toUpperCase()} <div class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full z-10"></div>`;

  const theme =
    state.currentProfile?.settings?.theme ||
  ...
  const showHeaderMiniApps =
    state.currentProfile?.settings?.show_header_miniapps;
  ...
  try {
    ...
    if (weekly.lastDayCounted !== todayStr && elapsedSeconds > 0) {
      ...
      if (weekly.todaySeconds >= 900) {
        ...
        // Grant Daily VIB
        const { error: grantError } = await supabase.rpc("system_grant_vib", {
          amount: bonusAmount,
          note: "Ежедневный бонус",
        });
        if (grantError) {
          console.error(
            "Failed to grant daily bonus (grantError):",
            grantError,
          );
          try {
            ...
            if (p) {
              await supabase
                .from("profiles")
                .update({ vib_balance: (p.vib_balance || 0) + bonusAmount })
                .eq("id", state.currentUser.id);
        ...
        import("./utils")
          .then((m) => m.customToast(`Бонус +${bonusAmount} VIB 🎉`))
          .catch(console.error);

        if (weekly.daysMet >= 7) {
          let weeklyBonusAmount = 50;
          const { error: wGrantError } = await supabase.rpc(
            "system_grant_vib",
            { amount: weeklyBonusAmount, note: "Бонус за 7 дней активности!" },
          );
          if (wGrantError) {
            try {
              const { data: p } = await supabase
              ...
              if (p) {
                await supabase
                  .from("profiles")
                  .update({
                    vib_balance: (p.vib_balance || 0) + weeklyBonusAmount,
                  })
                  .eq("id", state.currentUser.id);
                const res = await supabase
                  .from("vib_transfers")
                  .insert({
                    ...
                    message: "Бонус за 7 дней активности!",
                  });
                if (res.error) throw res.error;
              }
            } catch (fbErr) {}
          ...
          import("./utils")
            .then((m) =>
              m.customToast(
                `🔥 Поздравляем! Дополнительные +${weeklyBonusAmount} VIB за 7 дней активности!`,
              ),
            )
            .catch(console.error);
...
export async function logout() {
  if (state.isAdminStatus || (window as any).originalAdminUser) {
    import("./utils").then((m) =>
      m.customAlert(
        "Нельзя выйти из аккаунта в режиме инкогнито/симуляции. Используйте кнопку Exit.",
      ),
    );
    return;
  }
...
export async function deleteAccount() {
  if (state.isAdminStatus || (window as any).originalAdminUser) {
    import("./utils").then((m) =>
      m.customAlert("Нельзя удалить аккаунт в режиме инкогнито/симуляции."),
    );
    return;
  }
  if (state.currentProfile?.settings?.is_tech_support) {
    import("./utils").then((m) =>
      m.customAlert(
        "Аккаунты Тех. Поддержки не могут быть удалены. Для удаления, пожалуйста, попросите создателя снять с вас этот статус.",
      ),
    );
    return;
  }
