...
export const getFakeEmail = (nick: string) =>
  `${nick
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9]/g, "")}@vibegram.local`;
...
export function confirmPaidMessageModal(
  ...
  btn.onclick = () => {
    ...
    if (btn.getAttribute("data-clicks") === "0") {
      ...
      btn.className =
        "flex-1 px-4 py-3 bg-red-500 text-white rounded-2xl font-bold hover:bg-red-600 shadow-lg shadow-red-500/30 transition-all active:scale-95";
      document.getElementById("paid-msg-desc")!.innerHTML =
        `Списание <span class="font-bold text-red-500">${price} VIB</span> не подлежит возврату.`;
    } else {
      isConfirmed = true;
      btn.textContent = "...";
...
export function closeChatMobile(skipHistoryBack = false) {
  ...
  if (
    ...
    Object.keys(state.activeChatParentInfo).length > 0
  ) {
    ...
    const chatEl = document.querySelector(
      `#chats-list > div[data-chat-id="${parentId}"]`,
    ) as HTMLElement;
    if (chatEl) {
      chatEl.click();
      if (targetMsgId) {
        ...
        setTimeout(() => {
          import("./messages").then((m) =>
            m.loadMessagesUntil(parentId, targetMsgId),
          );
        }, 100);
      }
      return;
...
export function showInAppNotification(
  ...
  notif.className =
    "w-full bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-3 flex items-center gap-3 transform -translate-y-4 opacity-0 transition-all duration-300 pointer-events-auto cursor-pointer flex-shrink-0";
...
export function customToast(message: string) {
  ...
  alertDiv.className =
    "fixed top-4 left-1/2 transform -translate-x-1/2 bg-gray-800/90 backdrop-blur-sm text-white px-5 py-2.5 rounded-full shadow-lg z-[200] text-sm opacity-0 transition-opacity duration-300 pointer-events-none";
...
const getCloudinaryConfig = () => {
  ...
  return {
    cloudName:
      metaEnv.VITE_CLOUDINARY_CLOUD_NAME || process.env.CLOUDINARY_CLOUD_NAME,
    apiKey: metaEnv.VITE_CLOUDINARY_API_KEY || process.env.CLOUDINARY_API_KEY,
    apiSecret:
      metaEnv.VITE_CLOUDINARY_API_SECRET || process.env.CLOUDINARY_API_SECRET,
  };
};
...
async function generateCloudinarySignature(
  ...
  const stringToSign =
    keys.map((k) => `${k}=${paramsToSign[k]}`).join("&") + apiSecret;
...
export async function softDeleteCloudinaryFile(
  fileUrl: string,
): Promise<boolean> {
  try {
    ...
    try {
      ...
      if (data.resource_type === "image" || data.resource_type === "video") {
        if (isAvatar && data.resource_type === "image") {
          url = url.replace(
            "/upload/",
            `/upload/f_auto,q_auto,c_fill,g_face,w_256,h_256/`,
          );
        }
      }
...
(window as any).handleMediaError = function (el: HTMLElement, url: string) {
  ...
  if (retries < 3) {
    ...
    const errorText = isVideo
      ? "Видео повреждено"
      : "Файл поврежден (Кликните чтобы открыть)";
    const aspectClsMatcher = Array.from(el.classList).find((c) =>
      c.startsWith("aspect-"),
    );
    let aspectCls =
      aspectClsMatcher || "aspect-square max-w-[180px] sm:max-w-[220px]";

    let container = el.parentElement;
    if (
      isVideo &&
      container &&
      (container.classList.contains("chat-media-item-container") ||
        container.classList.contains("video-circle-container"))
    ) {
...
export async function sendPushNotification(
  ...
  try {
    ...
    if (targetTokens.length > 0) {
      console.log(
        "Invoking send-push edge function with " +
          targetTokens.length +
          " tokens...",
      );
...
export async function shareAppContent(
  chatIds: string[],
  mediaPayload: any,
