...
export const commentCountsMap: Record<string, number> = {};
        ...
        if (navigator.onLine) {
            try {
                const fetchPromise = supabase.from('messages')
                    .select('*, profiles(username, display_name, avatar_url, is_premium, premium_until)')
                    .eq('chat_id', chatId)
    ...
    messages.forEach((msg, i) => {
        ...
        const deleteBtnHtml = `
            <div class="relative ml-1 -mr-1 flex items-center">
                ...
                <div id="msg-menu-${msg.id}" class="msg-menu-dropdown absolute bottom-full ${isMe ? 'right-0' : 'left-0'} mb-1 hidden flex-col bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 z-50 w-48 py-1">
                    ...
                    <button onclick="event.stopPropagation(); copyMessageText(decodeURIComponent('${encodeURIComponent(displayContent).replace(/'/g, "%27")}'))" class="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors flex items-center gap-3">
        ...
        const innerHTML = `
            <div class="flex items-end max-w-full">
                ${avatarHtml}
                <div class="relative group select-none ${isMe ? 'bg-[#e3f2fd] dark:bg-blue-900/40 text-gray-900 dark:text-gray-100 rounded-[18px] rounded-br-[4px]' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-[18px] rounded-bl-[4px]'} p-2 px-3 shadow-sm border border-gray-200/60 dark:border-gray-700/60 max-w-full flex-1 min-w-0" style="touch-action: pan-y;" id="msg-${msg.id}" data-copy-content="${encodeURIComponent(displayContent).replace(/'/g, "%27")}" data-reply-content="${encodedContentLabel}" data-reply-sender="${encodeURIComponent(displaySenderName).replace(/'/g, "%27")}" ontouchstart="handleMessageTouchStart(event, '${msg.id}')" ontouchend="handleMessageTouchEnd(event)" ontouchcancel="handleMessageTouchEnd(event)" ontouchmove="handleMessageTouchMove(event)" onmousedown="handleMessageTouchStart(event, '${msg.id}')" onmouseup="handleMessageTouchEnd(event)" onmousemove="handleMessageTouchMove(event)" onmouseleave="handleMessageTouchEnd(event)">
                    ${shareHtml}
                    ${forwardHtml}
                    ${replyHtml}
                    ...
                    <div class="mt-2 pt-2 border-t border-gray-200/50 dark:border-gray-700/50 flex w-full">
                        <button onclick="event.stopPropagation(); window.openComments('${msg.id}')" class="flex-1 flex items-center justify-center gap-2 text-[13px] font-medium text-blue-500/90 dark:text-blue-400/90 hover:bg-blue-50/50 dark:hover:bg-blue-900/20 rounded-md transition-colors py-1 relative">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                            Комментарии
                            <span class="comment-count-badge ${commentCountsMap[msg.id] ? '' : 'hidden'} ml-1 bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 text-[11px] font-bold px-1.5 py-0.5 rounded-full" data-post-id="${msg.id}">${commentCountsMap[msg.id] || 0}</span>
                        </button>
                    </div>
                    ` : ''}
                </div>
            ...
            try {
                ...
                if (data) {
                    ...
                    for (const badge of commentBadges) {
                        ...
                        commentCountsMap[badge.dataset.postId!] = count;
                        if (count > 0) {
