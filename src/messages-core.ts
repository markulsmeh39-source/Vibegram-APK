...
export async function markMessagesAsRead(chatId: string) {
  ...
  const unreadBadge = document.querySelector(
    `#chats-list > div[data-chat-id="${chatId}"] .bg-blue-500.text-white.rounded-full`,
  );
  if (unreadBadge) {
    unreadBadge.remove();
  }
  ...
  messageScrollListener = async () => {
    ...
    if (floatingDate) {
      ...
      for (let i = 0; i < elements.length; i++) {
        ...
        if (
          (el.classList.contains("msg-wrapper") ||
      ...
      if (topMsgOrDivider) {
        ...
        if (dateStr) {
          ...
          if ((window as any).floatingDateTimeout)
            clearTimeout((window as any).floatingDateTimeout);
...
export async function loadMessages(chatId: string, isInitialLoad = false) {
  ...
  try {
    ...
    const list = document.getElementById("messages-list")!;
    list.innerHTML =
      '<div class="flex h-full items-center justify-center"><div class="bg-red-500/80 text-white px-5 py-2 rounded-full text-sm font-medium backdrop-blur-md shadow-lg">Ошибка загрузки сообщений</div></div>';
  }
}
export function renderMessages(messages: any[], isInitialLoad = false) {
  ...
  const isScrolledUp =
    !isInitialLoad &&
    Math.abs(list.scrollHeight - list.scrollTop - list.clientHeight) > 20;
  ...
  if (isScrolledUp) {
    ...
    for (let child of children) {
      if (child.id && child.id.startsWith("msg-wrapper-")) {
        ...
        if (offset >= -50) {
          // Slight negative threshold to catch partially hidden elements
          anchorElement = child;
          anchorOffset = offset;
          break;
        }
  ...
  if (messages.length === 0) {
    list.innerHTML =
      '<div class="flex h-full items-center justify-center"><div class="bg-black/40 text-white px-5 py-2 rounded-full text-sm font-medium backdrop-blur-md shadow-lg">Здесь пока пусто. Напишите первым!</div></div>';
    return;
  }

  if (
    list.children.length === 1 &&
    list.children[0].textContent?.includes("Здесь пока пусто")
  ) {
    list.innerHTML = "";
  }
  ...
  messages.forEach((msg, i) => {
    ...
    const dateStr = msgDateObj
      .toLocaleDateString("ru-RU", {
        month: "long",
        day: "numeric",
        year:
          msgDateObj.getFullYear() !== new Date().getFullYear()
            ? "numeric"
            : undefined,
      })
      .replace(" г.", "");
    ...
    if (dateStr !== currentDateString) {
      ...
      dateDiv.id = dateId;
      dateDiv.className =
        "w-full flex justify-center my-3 msg-date-divider shrink-0";
    ...
    const actualMedia = mediaArr.filter(
      (m: any) =>
        m.type !== "reply" &&
        m.type !== "forward" &&
        m.type !== "admin_mode" &&
        m.type !== "comments_enabled",
    );
    ...
    const isCommentsEnabled = mediaArr.some(
      (m: any) => m.type === "comments_enabled",
    );
    ...
    if (msg.message_type === "poll" && actualMedia.length > 0) {
      ...
      pollData.options.forEach((opt: any) => {
        const percent =
          totalVotes > 0
            ...
            : 0;
        const isVoted = state.currentUser
          ? opt.votes.includes(state.currentUser.id)
          : false;
      ...
      const transcribeBtn = !actualMedia[0].transcription
        ? `<button id="transcribe-btn-${msg.id}" onclick="transcribeMedia('${actualMedia[0].url}', '${msg.id}', 'voice')" class="w-7 h-7 shrink-0 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 rounded-full flex items-center justify-center transition-colors ml-1" title="Расшифровать"><span class="text-[10px] font-bold">Aa</span></button>`
        : "";
      ...
      const transcribeBtn = !actualMedia[0].transcription
        ? `<button id="transcribe-btn-${msg.id}" onclick="transcribeMedia('${actualMedia[0].url}', '${msg.id}', 'video_circle')" class="absolute bottom-0 right-0 w-8 h-8 bg-gray-800/70 hover:bg-gray-800 text-white rounded-full flex items-center justify-center transition-colors z-10 backdrop-blur-sm" title="Расшифровать"><span class="text-[11px] font-bold">Aa</span></button>`
        : "";
      ...
      actualMedia[0].type?.startsWith("audio/")
    ) {
      ...
      actualMedia.forEach((file: any) => {
        ...
        if (file.ratio === "16/9")
          aspectCls = "aspect-[16/9] max-w-[240px] sm:max-w-[300px]";
        else if (file.ratio === "9/16")
          aspectCls = "aspect-[9/16] max-w-[140px] sm:max-w-[180px]";
        else if (file.ratio === "1/1")
          aspectCls = "aspect-square max-w-[180px] sm:max-w-[220px]";
    ...
    if (
      ...
      if (adminModeData) {
        ...
        const isMember = state.activeChatMembers?.some(
          (m: any) => m.user_id === msg.sender_id,
        );
        if (
          !isMember &&
          msg.profiles?.settings?.is_tech_support &&
    ...
    if (
      ...
      window.location.hash.includes("TECH_SUPPORT_CHAT")
    ) {
    ...
    const isPremium =
      msg.profiles?.is_premium &&
      (!msg.profiles.premium_until ||
        new Date(msg.profiles.premium_until) > new Date());
    const premiumBadge = isPremium
      ? `<span class="inline-flex items-center justify-center ml-1" title="Vibegram Premium"><img src="./image/Google-Gemini-Logo-Transparent.png" class="w-3.5 h-3.5 object-contain" alt="Premium"></span>`
      : "";

    const senderNameHtml =
      state.activeChatType === "group" && (!isMe || isSystemAdmin)
        ? `<div onclick="event.stopPropagation(); openUserProfile('${msg.sender_id}')" class="text-[13px] font-bold text-blue-500 mb-0.5 flex items-center cursor-pointer hover:underline w-fit">${displaySenderName}${premiumBadge}</div>`
        : "";
    ...
    const encodedContent = encodeURIComponent(msg.content || "").replace(
      /'/g,
      "%27",
    );
  ...
  if (isInitialLoad) {
    const scrollState = state.activeChatId
      ? state.chatScrollPositions.get(state.activeChatId)
      : undefined;
    ...
    const applyScrollPos = () => {
      if (anchorElement && list.contains(anchorElement)) {
        ...
        list.scrollTop =
          list.scrollHeight - previousScrollHeight + previousScrollTop;
      }
    };
    applyScrollPos();
    updateUnreadFloatingCounter(messages);
  ...
  setTimeout(async () => {
    const commentBadges = Array.from(
      document.querySelectorAll(".comment-count-badge"),
    ) as HTMLElement[];
...
function updateUnreadFloatingCounter(messages: any[]) {
  ...
  if (!counter) {
    ...
    counter.className =
      "absolute bottom-20 right-6 bg-blue-500 text-white w-10 h-10 rounded-full flex items-center justify-center shadow-lg cursor-pointer hover:bg-blue-600 transition-colors z-40 animate-bounce";
  ...
  const unreadCount = messages.filter(
    (m) => !m.is_read && m.sender_id !== state.currentUser.id,
  ).length;
  if (unreadCount > 0) {
    counter.classList.remove("hidden");
    document.getElementById("unread-floating-count")!.textContent =
...
export function handleInput() {
  ...
  document
    ...
    .classList.toggle("hidden", !isAiMode);
  document
    .getElementById("send-btn")!
    .classList.toggle("hidden", !hasContent || isAiMode);
...
export async function sendMessage() {
  ...
  if (
    ...
    if (!iAmPremium && targetIsPremium && price > 0) {
      ...
      confirmPaidMessageModal(
        price,
        state.activeChatOtherUser.display_name ||
          state.activeChatOtherUser.username,
        async () => {
          // Proceed to pay
          try {
            const { error } = await supabase.rpc("transfer_vib", {
              receiver_id:
                state.activeChatOtherUser.user_id ||
            ...
            if (error) {
              if (error.message.includes("not exist")) {
                await supabase.rpc("transfer_vib", {
                  receiver_id:
                    state.activeChatOtherUser.user_id ||
                    state.activeChatOtherUser.id,
                  amount: price,
...
async function actuallySend(
  ...
  try {
    ...
    if (actualMediaCount > 0) {
      if (mediaArr[0].type?.startsWith("image/") && !mediaArr[0].asFile)
        messageType = "photo";
      else if (mediaArr[0].type?.startsWith("video/") && !mediaArr[0].asFile)
        messageType = "video";
    ...
    if (state.activeChatId) {
      import("./messages-core").then((m) =>
        m.loadMessages(state.activeChatId!),
      );
      ...
      import("./supabase").then(async (s) => {
        ...
        if (!notificationBody) {
          if (actualMediaCount > 0) {
            ...
            const hasDocument = uploadedMedia.some(
              (m: any) =>
                m.asFile ||
                (!m.type?.startsWith("image/") &&
                  !m.type?.startsWith("video/")),
            );
            const hasVideo = uploadedMedia.some(
              (m: any) => !m.asFile && m.type?.startsWith("video/"),
            );
            const hasPhoto = uploadedMedia.some(
              (m: any) => !m.asFile && m.type?.startsWith("image/"),
            );
        ...
        if (state.activeChatType === "channel") {
          const groupName = document
            .getElementById("current-chat-name")
            ?.innerText?.trim();
    ...
    if (
      ...
      document.getElementById(tempId)?.remove();
      customAlert(
        "Ошибка при отправке: " + (err.message || JSON.stringify(err)),
      );
    }
  }
...
export function cancelSend(tempId: string) {
  if (
    (window as any).pendingUploads &&
    (window as any).pendingUploads[tempId]
  ) {
