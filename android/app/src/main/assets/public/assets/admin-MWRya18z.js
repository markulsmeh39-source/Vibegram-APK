const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/index-y8-y7piH.js","assets/index-Bk01aTlT.css"])))=>i.map(i=>d[i]);
import{s as c,a as o,c as w,_ as m}from"./index-y8-y7piH.js";function U(){const s=document.getElementById("modal-content");s.innerHTML=`
    ...
    `,window._adminIsCreator=s,g()}let _=[];window.filterAdminUsers=s=>{const e=s.toLowerCase(),t=_.filter(r=>(r.display_name||"").toLowerCase().includes(e)||(r.username||"").toLowerCase().includes(e));A(t)};let f={reset_auth:!1,analytics:!1};async function g(){var s;try{const{data:e,error:t}=await o.from("profiles").select("*"),{data:r,error:n}=await o.from("chats").select("id, type, title, created_at, avatar_url, description");(t||n)&&console.warn("Some data could not be fetched due to RLS. Best effort displayed.");const a=e||[];_=a;const i=a.find(p=>{var I;return(I=p.settings)==null?void 0:I.is_tech_support});i&&((s=i.settings)!=null&&s.support_permissions)&&(f={...i.settings.support_permissions});const d=document.getElementById("ts-perm-reset"),l=document.getElementById("ts-perm-analytics");d&&(d.checked=f.reset_auth),l&&(l.checked=f.analytics);const u=r||[];x=u;const v=u.filter(p=>p.type==="private"&&p.description!=="TECH_SUPPORT_CHAT").length,y=u.filter(p=>p.type==="group").length,b=u.filter(p=>p.type==="channel").length,T=`
            ...
            <div class="bg-gray-900 p-4 rounded-lg flex flex-col justify-center items-center">
                <span class="text-3xl font-bold text-green-400">${y}</span>
            ...
            <div class="flex items-center justify-between w-full">
                <div class="flex items-center gap-3 w-1/3 min-w-0 pr-4">
                    <div class="relative w-10 h-10 shrink-0">
                        ...
                        ${y?'<div class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-gray-800 rounded-full z-40"></div>':""}
                    </div>
