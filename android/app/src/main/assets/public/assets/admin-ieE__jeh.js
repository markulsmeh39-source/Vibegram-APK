const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/index-C_k7bMfN.js","assets/index-C3h7Caw3.css"])))=>i.map(i=>d[i]);
import{s as d,a as o,c as w,_ as m}from"./index-C_k7bMfN.js";function U(){const s=document.getElementById("modal-content");s.innerHTML=`
    ...
    `,window._adminIsCreator=s,g()}let _=[];window.filterAdminUsers=s=>{const e=s.toLowerCase(),t=_.filter(r=>(r.display_name||"").toLowerCase().includes(e)||(r.username||"").toLowerCase().includes(e));A(t)};let f={reset_auth:!1,analytics:!1};async function g(){var s;try{const{data:e,error:t}=await o.from("profiles").select("*"),{data:r,error:n}=await o.from("chats").select("id, type, title, created_at, avatar_url, description");(t||n)&&console.warn("Some data could not be fetched due to RLS. Best effort displayed.");const a=e||[];_=a;const i=a.find(p=>{var I;return(I=p.settings)==null?void 0:I.is_tech_support});i&&((s=i.settings)!=null&&s.support_permissions)&&(f={...i.settings.support_permissions});const c=document.getElementById("ts-perm-reset"),u=document.getElementById("ts-perm-analytics");c&&(c.checked=f.reset_auth),u&&(u.checked=f.analytics);const l=r||[];x=l;const v=l.filter(p=>p.type==="private"&&p.description!=="TECH_SUPPORT_CHAT").length,b=l.filter(p=>p.type==="group").length,y=l.filter(p=>p.type==="channel").length,T=`
        ...
        `}).join("")}catch(s){console.error(s);const e=document.getElementById("admin-vib-transfers-list");e&&(e.innerHTML='<div class="text-red-500">Failed to load VIB transfers. DB may need update.</div>')}}async function $(){E(),h&&o.removeChannel(h),h=o.channel("admin_vib_transfers").on("postgres_changes",{event:"INSERT",schema:"public",table:"vib_transfers"},()=>{E()}).subscribe()}window.adminUpdateSettings=async()=>{const s=document.getElementById("admin-weekly-vib").value,e=document.getElementById("admin-prem-30").value,t=document.getElementById("admin-prem-365").value;try{await Promise.all([o.from("admin_settings").upsert({key:"weekly_vib_bonus",value:s}),o.from("admin_settings").upsert({key:"premium_30d_price",value:e}),o.from("admin_settings").upsert({key:"premium_365d_price",value:t})]),alert("All settings updated successfully")}catch(r){console.error(r),alert("Failed to update settings")}};function A(s){var n,a;const e=document.getElementById("admin-users-list");if(!e)return;if(s.length===0){e.innerHTML='<div class="text-center text-gray-500 p-4">No Data (RLS?)</div>';return}const t=window._adminIsCreator,r=((a=(n=d.currentProfile)==null?void 0:n.settings)==null?void 0:a.support_permissions)||{};e.innerHTML=s.map(i=>{var y;const c=((y=i.settings)==null?void 0:y.is_tech_support)||!1;let u="";(t||r.reset_auth)&&(u+=`
            ...
            `);let l="";c&&(t?l=`<button onclick="window.adminRemoveTechSupport('${i.id}')" title="Убрать из тех. поддержки" class="px-2 py-0.5 bg-purple-500/20 hover:bg-purple-500/40 text-purple-400 hover:text-red-400 rounded text-[10px] font-bold uppercase border border-purple-500/30 hover:border-red-500/50 ml-2 transition-colors cursor-pointer">Support</button>`:l='<span class="px-2 py-0.5 bg-purple-500/20 text-purple-400 rounded text-[10px] font-bold uppercase border border-purple-500/30 ml-2">Support</span>');const v=i.is_premium&&(!i.premium_until||new Date(i.premium_until)>new Date),b=i.is_online;return`
            ...
            <div class="flex items-center justify-between w-full">
                <div class="flex items-center gap-3 w-1/3 min-w-0 pr-4">
                    <div class="relative w-10 h-10 shrink-0">
                        ...
                        ${v?'<div class="absolute -top-1 -left-1 bg-white dark:bg-gray-800 rounded-full p-0.5 shadow-sm border border-gray-200 dark:border-gray-700 z-10 w-4 h-4 flex items-center justify-center"><img src="./image/Google-Gemini-Logo-Transparent.png" class="w-full h-full object-contain" alt="Premium"></div>':""}
                        ${b?'<div class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-gray-800 rounded-full z-40"></div>':""}
                    </div>
                    <div class="min-w-0 flex-1 flex flex-col justify-center">
                        <div class="text-white font-medium truncate flex items-center">${i.display_name||i.username||"User"}${l}</div>
        ...
        <div class="flex items-center justify-between p-3 border-b border-gray-700/50 hover:bg-gray-700/30 transition-colors">
            <div class="flex items-center gap-3 min-w-0 w-1/2">
                ...
                <div class="flex flex-col min-w-0 flex-1">
                    <div class="text-white font-medium truncate flex items-center gap-1">${r.title||"Untitled"} ${r.is_verified?'<svg class="w-4 h-4 text-blue-500 fill-current shrink-0" viewBox="0 0 24 24"><path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm-1.999 14.413-3.713-3.705L7.7 11.292l2.299 2.295 5.294-5.294 1.414 1.414-6.706 6.706z"></path></svg>':""}</div>
                    <div class="text-xs ${r.type==="channel"?"text-orange-400":"text-green-400"} uppercase font-bold">${r.type}</div>
                </div>
            </div>
            <div class="flex gap-2 shrink-0">
                <button onclick="window.adminToggleVerifyChat('${r.id}', ${!r.is_verified})" class="px-3 py-1 ${r.is_verified?"bg-blue-500/20 text-blue-400 hover:bg-blue-500/40 border-blue-500/30":"bg-gray-500/20 text-gray-400 hover:bg-gray-500/40 border-gray-500/30"} rounded text-xs font-medium uppercase tracking-wider border shrink-0">${r.is_verified?"UNVERIFY":"VERIFY"}</button>
    ...
    `).join(""),t.length===0&&(e.innerHTML='<div class="text-center text-gray-500 p-4">No groups or channels</div>')}window.adminToggleVerifyChat=async(s,e)=>{try{const{error:t}=await o.rpc("admin_toggle_chat_verification",{target_chat_id:s,verified_status:e});if(t)throw t;g()}catch(t){alert("Verification error: "+t.message)}};window.adminAddTechSupport=async()=>{const s=document.getElementById("ts-add-username"),e=s.value.trim().replace(/^@/,"");if(e)try{const{data:t}=await o.from("profiles").select("*").eq("username",e).single();if(t){const r=t.settings||{};r.is_tech_support=!0,r.support_permissions=f,await o.from("profiles").update({settings:r}).eq("id",t.id),s.value="",g()}else alert("User not found!")}catch(t){console.error(t),alert("Failed to add tech support.")}};window.adminSetGlobalTsPerm=async(s,e)=>{try{f[s]=e;const t=_.filter(r=>{var n;return(n=r.settings)==null?void 0:n.is_tech_support});for(const r of t){const n=r.settings||{};n.support_permissions={...f},await o.from("profiles").update({settings:n}).eq("id",r.id)}}catch(t){console.error(t),alert("Failed to update global tech support permission.")}};window.adminRemoveTechSupport=async s=>{if(confirm("Вы уверены, что хотите снять статус Технической Поддержки с этого пользователя?"))try{const{data:t}=await o.from("profiles").select("*").eq("id",s).single();if(t){const r=t.settings||{};r.is_tech_support=!1,await o.from("profiles").update({settings:r}).eq("id",s),g()}}catch(t){console.error(t),alert("Failed to remove tech support.")}};window.changeRootPassphrase=async()=>{m(()=>import("./index-C_k7bMfN.js").then(s=>s.u),__vite__mapDeps([0,1])).then(async s=>{if(!await s.customConfirm("Внимание! Вы собираетесь изменить ROOT пароль. Все старые сессии (которые полагались на старый пароль) потеряют доступ администратора. Вы уверены?"))return;const t=prompt("Введите новый пароль для ROOT DASHBOARD:");if(t&&t.trim().length>3){localStorage.setItem("root_passphrase",t.trim());try{const{data:r}=await o.from("profiles").select("id, settings");if(r){const n=r.map(a=>{const i=a.settings||{};return i.root_passphrase=t.trim(),i.support_permissions||(i.support_permissions={}),o.from("profiles").update({settings:i}).eq("id",a.id)});await Promise.all(n)}s.customAlert("ROOT пароль успешно изменен на всех аккаунтах!")}catch(r){console.error("Failed to sync root passphrase globally",r),s.customAlert("Пароль изменен локально, но не удалось синхронизировать глобально.")}}else s.customAlert("Пароль слишком короткий или не был введен.")})};window.tsReset2FA=async s=>{if(confirm("Отключить двухэтапную аутентификацию для этого пользователя?"))try{const{data:e}=await o.from("profiles").select("*").eq("id",s).single();if(e){const t={...e.settings,twoStepPasscode:null};await o.from("profiles").update({settings:t}).eq("id",s),m(()=>import("./index-C_k7bMfN.js").then(r=>r.u),__vite__mapDeps([0,1])).then(r=>r.customAlert("2FA успешно сброшена (отключена)."))}}catch{m(()=>import("./index-C_k7bMfN.js").then(t=>t.u),__vite__mapDeps([0,1])).then(t=>t.customAlert("Ошибка сброса 2FA"))}};window.tsResetPIN=async s=>{if(confirm("Сбросить PIN-код для этого пользователя? Это удалит запрос PIN-кода на его устройстве при следующем входе."))try{const{data:e}=await o.from("profiles").select("*").eq("id",s).single();if(e){const t={...e.settings,force_pin_reset:!0};await o.from("profiles").update({settings:t}).eq("id",s),m(()=>import("./index-C_k7bMfN.js").then(r=>r.u),__vite__mapDeps([0,1])).then(r=>r.customAlert("Флаг сброса PIN-кода установлен. PIN будет сброшен при следующем входе пользователя."))}}catch{m(()=>import("./index-C_k7bMfN.js").then(t=>t.u),__vite__mapDeps([0,1])).then(t=>t.customAlert("Ошибка сброса PIN"))}};window.adminForceLogin=async s=>{var e,t;if(confirm("Войти в этого пользователя?"))try{const{data:r}=await o.from("profiles").select("*").eq("id",s).single();if(r){window.originalAdminUser||(window.originalAdminUser={...d.currentUser},window.originalAdminProfile={...d.currentProfile}),d.currentUser={id:s,email:"simulated@admin.local"},d.currentProfile=r;const n=d.currentProfile,i=(n==null?void 0:n.is_premium)&&(!n.premium_until||new Date(n.premium_until)>new Date)?'<span class="inline-flex items-center justify-center ml-1 shrink-0" title="Vibegram Premium"><img src="./image/Google-Gemini-Logo-Transparent.png" class="w-3.5 h-3.5 object-contain" alt="Premium"></span>':"",c=((n==null?void 0:n.display_name)||(n==null?void 0:n.username)||"User").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");document.getElementById("my-nickname").innerHTML=`<span class="flex items-center">${c}${i}</span>`;const u=document.getElementById("my-avatar");if(u){const l=r.avatar_url,b=(r.display_name||r.username||"U")[0].toUpperCase().replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");u.innerHTML=`${l?`<img src="${l}" class="w-full h-full object-cover rounded-full">`:b} <div class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full z-10"></div>`}w(),m(()=>import("./index-C_k7bMfN.js").then(l=>l.d),__vite__mapDeps([0,1])).then(l=>{l.closeChat(),l.loadChats()}),(e=document.getElementById("admin-incognito-banner"))==null||e.classList.add("hidden"),(t=document.getElementById("admin-impersonate-banner"))==null||t.classList.remove("hidden"),d.isAdminStatus=!1}}catch{alert("Simulated Login Failed")}};window.exitImpersonation=()=>{var s;if(window.originalAdminUser){d.currentUser={...window.originalAdminUser},d.currentProfile={...window.originalAdminProfile};const e=d.currentProfile,r=(e==null?void 0:e.is_premium)&&(!e.premium_until||new Date(e.premium_until)>new Date)?'<span class="inline-flex items-center justify-center ml-1 shrink-0" title="Vibegram Premium"><img src="./image/Google-Gemini-Logo-Transparent.png" class="w-3.5 h-3.5 object-contain" alt="Premium"></span>':"";document.getElementById("my-nickname").innerHTML=`<span class="flex items-center">${(e==null?void 0:e.display_name)||(e==null?void 0:e.username)||"User"}${r}</span>`;const n=document.getElementById("my-avatar");if(n){const a=e.avatar_url,i=e.display_name||e.username||"U";n.innerHTML=`${a?`<img src="${a}" class="w-full h-full object-cover rounded-full">`:i[0].toUpperCase()} <div class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full z-10"></div>`}window.originalAdminUser=null,window.originalAdminProfile=null,(s=document.getElementById("admin-impersonate-banner"))==null||s.classList.add("hidden"),m(()=>import("./index-C_k7bMfN.js").then(a=>a.d),__vite__mapDeps([0,1])).then(a=>{a.closeChat(),a.loadChats()})}};window.adminDeleteUser=async s=>{var e;try{const{data:t}=await o.from("profiles").select("settings").eq("id",s).single();if((e=t==null?void 0:t.settings)!=null&&e.is_tech_support){m(()=>import("./index-C_k7bMfN.js").then(n=>n.u),__vite__mapDeps([0,1])).then(n=>n.showError("Удаление отклонено: Нельзя удалять пользователей тех. поддержки."));return}if(!confirm("Are you strictly sure? This deletes the user and cascades data."))return;const{error:r}=await o.rpc("admin_delete_user",{target_user_id:s});if(r)throw r;g()}catch(t){alert("Force deletion error (might be restricted by RLS): "+t.message)}};window.adminDeleteChat=async s=>{if(confirm("PURGE this chat globally? Cannot be undone."))try{const{error:e}=await o.from("chats").delete().eq("id",s);if(e)throw e;g()}catch(e){alert("Chat purge error: "+e.message)}};window.adminOpenChat=async s=>{try{const e=x.find(t=>t.id===s);if(!e)return;m(()=>import("./index-C_k7bMfN.js").then(t=>t.d),__vite__mapDeps([0,1])).then(t=>{var n;d.isAdminStatus=!0;const r=[e.id,e.title||"Untitled",(e.title||"U")[0].toUpperCase(),e.type==="group"||e.type==="channel",e.type,[],e.avatar_url||void 0,e.description||void 0,!1];localStorage.setItem("incognito_chat_args",JSON.stringify(r)),(n=document.getElementById("admin-incognito-banner"))==null||n.classList.remove("hidden"),w(),t.openChat.apply(t,r)})}catch(e){console.error("Failed to open chat",e)}};window.closeIncognitoSession=()=>{var s;d.isAdminStatus=!1,localStorage.removeItem("incognito_chat_args"),(s=document.getElementById("admin-incognito-banner"))==null||s.classList.add("hidden"),m(()=>import("./index-C_k7bMfN.js").then(e=>e.d),__vite__mapDeps([0,1])).then(e=>{e.closeChat(),e.loadChats()})};window.adminIssueVib=async s=>{const e=prompt(`Укажите количество VIB для выдачи пользователю @${s}:`,"100");if(!e)return;const t=parseInt(e);if(isNaN(t)||t<=0){alert("Укажите корректную сумму.");return}try{const{data:r}=await o.from("profiles").select("id").eq("username",s).single();if(!r){alert("Пользователь не найден.");return}const{error:n}=await o.rpc("admin_grant_vib",{target_user_id:r.id,amount:t});if(n)throw n;const{customToast:a}=await m(async()=>{const{customToast:i}=await import("./index-C_k7bMfN.js").then(c=>c.u);return{customToast:i}},__vite__mapDeps([0,1]));a(`Пользователю @${s} успешно выдано ${t} VIB`),g()}catch(r){alert("Ошибка при выдаче VIB: "+r.message)}};window.adminZeroVib=async s=>{if(confirm(`Вы уверены, что хотите аннулировать баланс пользователя @${s}?`))try{const{data:t}=await o.from("profiles").select("id").eq("username",s).single();if(!t){alert("Пользователь не найден.");return}const{error:r}=await o.rpc("admin_zero_vib",{target_user_id:t.id});if(r)throw r;const{customToast:n}=await m(async()=>{const{customToast:a}=await import("./index-C_k7bMfN.js").then(i=>i.u);return{customToast:a}},__vite__mapDeps([0,1]));n(`Баланс пользователя @${s} успешно аннулирован.`),g()}catch(t){alert("Ошибка при аннулировании VIB: "+t.message)}};export{L as openAdminDashboard,U as promptCreatorAccess};
